  ...
  fetch("/api/users")
    ...
    .then(response => {
      ...
      response.forEach(person => {
        ...
        tr.innerHTML +=
          "<td style = 'text-align: center'><a href='#' class = 'edit'>Edit</a> | <a href='#' class = 'remove'>Remove</a></td>";
    ...
    .then(() => {
      ...
      links.forEach((link) => link.addEventListener("click", deleteUser));
    });
};
...
const deleteUser = (e) => {
  const id = e.target.closest("tr").children[0].innerHTML;
  fetch(`/api/users/:${id}`, {
    method: "delete",
    headers: new Headers({
      Accept: "application/json",
